...
const CartPopup: React.FC<CartPopupProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <>
      ...
      <div className={`z-50 fixed top-0 right-0 h-full w-[456px] bg-white  shadow-lg p-6 transition-transform ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        ...
        {items.map((data, index) => (
          ...
          <CartItem key={index} img={data.image} title={data.name} price={data.price}/>
          <div
        className="border rounded-full items-center w-[30px] h-[30px]"
